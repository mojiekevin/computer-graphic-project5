...
public class RenderCamera extends RenderObject {
	...
	public void updateCameraMatrix(Vector2 viewportSize) {
		...
		if(sceneCamera.imageSize.x / sceneCamera.imageSize.y < viewportSize.x / viewportSize.y){
			sceneCamera.imageSize.x = sceneCamera.imageSize.y * viewportSize.x / viewportSize.y;
		}
		else{
			sceneCamera.imageSize.y = sceneCamera.imageSize.x / (viewportSize.x / viewportSize.y);
		}

		// Create Projection
		if(sceneCamera.isPerspective){
			Matrix4 Mper = Matrix4.createPerspective((float)sceneCamera.imageSize.x, (float)sceneCamera.imageSize.y, (float)sceneCamera.zPlanes.x, (float)sceneCamera.zPlanes.y);
			mProj.set(Mper);
		}
		else{
			Matrix4 Morth = Matrix4.createOrthographic((float)sceneCamera.imageSize.x, (float)sceneCamera.imageSize.y, (float)sceneCamera.zPlanes.x, (float)sceneCamera.zPlanes.y);
			mProj.set(Morth);
		}
